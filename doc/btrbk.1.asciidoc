...
all subvolumes in the set.
+
If called with the '--related' option, btrbk also lists all related
subvolumes. Note that while this is usually fine for snapshots,
parent-uuid relations break for received subvolumes as soon as an
intermediate subvolume is deleted.
